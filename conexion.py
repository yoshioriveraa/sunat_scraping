...
def save_to_sql(df_1, df_2, db_name= r'bdd\database.db'):
    """
    ...
    try:
        if df_1 is not None and df_2 is not None:
            try:
                ...
                df_1.to_sql('tabla1', conn, if_exists='replace', index=False)
                df_2.to_sql('tabla2', conn, if_exists='replace', index=False)
